...
;       IRIS_XCONTROL
; PURPOSE:
;	IRIS_XCONTROL is the main QL control window. After selecting a
...
;
; CALLING SEQUENCE:
;       iris_xcontrol, data, group_leader = group 
;
; INPUTS:
;	data: data object of type 'irs_data'
;
; KEYWORD PARAMETERS:
...
;
;
pro iris_xcontrol_destroy, event
  pseudoevent = {WIDGET_KILL_REQUEST, $
    ID:event.id, $
    TOP:event.top, $
    HANDLER:event.handler}
  iris_xcontrol_event, pseudoevent
end

pro iris_xcontrol_cleanup, tlb
   widget_control, tlb, get_uvalue = info
   ptr_free,(*info).d
   ptr_free,info
end

pro iris_xcontrol_event, event
...
pro iris_xcontrol_get_data_info, info
  nwin=*(*info).d->getnwin()
...
pro iris_xcontrol_printfilename, event
  widget_control, event.top, get_uvalue = info
  print,(*(*info).d)->getfilename()
end

; Display Header in text window
pro iris_xcontrol_hdrdisp,event
  widget_control, event.top ,get_uvalue = info
  ...
  closebutton = widget_button(closefield, value = 'Close', $
                              event_pro = 'iris_xcontrol_hdrdisp_destroy')
...
pro iris_xcontrol_hdrdisp_destroy, event
 widget_control, event.top,/destroy
end

function iris_xcontrol_lineselect_fuv, event
  widget_control, event.top, get_uvalue = info
...
function iris_xcontrol_lineselect_nuv, event
  widget_control, event.top, get_uvalue = info
...
pro iris_xcontrol_lineselect_sji, event
  widget_control, event.top, get_uvalue = info
...
; Display Header in text window
pro iris_xcontrol_leve3file,event
  widget_control, event.top ,get_uvalue = info
  ...
  if N_ELEMENTS(level3_dir) gt 0 && level3_dir ne '' then begin
    ...
    save,level3_dir,file=IRISxfiles_appReadme()+'/iris_xcontrol_lev3dir.tmp'
    widget_control,(*info).lev3filemenu,set_value='Change level 3 dir: '+level3_dir+' '
  endif
end

function iris_xcontrol_replace_level3,event
  widget_control, event.top ,get_uvalue = info
...
pro iris_xcontrol_level3, event
  widget_control, event.top, get_uvalue = info
  if (*info).level3_dir eq '' then cd,current=wdir else wdir=(*info).level3_dir
...
function iris_xcontrol_dispselect, event
  widget_control, event.top, get_uvalue = info
...
pro iris_xcontrol_listmoments, event
  widget_control, event.top, get_uvalue = info
  thisevent=tag_names(event,/structure_name)
  case thisevent of
    'WIDGET_DROPLIST': begin
  ...
  case mode of
    ...
    1: begin
         iris_xcontrol_moments, 'moment', event
       end
    2: begin
         iris_xcontrol_moments, 'gauss', event
       end
    3: begin
         iris_xcontrol_moments, 'dgf', event
       end
    else: message,'iris_xcontrol_listmoments: Impossible mode!!',/info
  endcase
end

pro iris_xcontrol_detnuv, event
  widget_control, event.top, get_uvalue = info
...
pro iris_xcontrol_detfuv, event
  widget_control, event.top, get_uvalue = info
  case event.release of 
  0: begin
       bin_sp=*(*info).d->binning_spectral()
...
pro iris_xcontrol_shownuv1, event
  widget_control, event.top, get_uvalue = info
...
pro iris_xcontrol_shownuv2, event
  widget_control, event.top, get_uvalue = info
...
pro iris_xcontrol_showfuv1, event
  widget_control, event.top, get_uvalue = info
...
pro iris_xcontrol_showfuv2, event
  widget_control, event.top, get_uvalue = info
...
pro iris_xcontrol_pointing, event
  widget_control, event.top, get_uvalue = info
...
; calculate moments
pro iris_xcontrol_moments, fit, event   
  widget_control, event.top, get_uvalue = info
...
pro iris_xcontrol, data, group_leader = group_leader

  if n_params() lt 1 then begin
    message,'iris_xcontrol, data, group_leader = group, filename = filename',/cont
    return
  endif
  ...
  tlb = widget_base(/row, mbar=menubar, $
		  title='IRIS_Xcontrol - '+data->getfilename(), $
...
;
  level3dirfile = IRISxfiles_appReadme()+'/iris_xcontrol_lev3dir.tmp'
...
; create menu bar:
  ...
  closemenu=widget_button(filemenu, value='Close', $
			 event_pro='iris_xcontrol_destroy') 
  ...
  lev3filemenu=widget_button(optmenu, value='Change level 3 dir: '+level3_dir+' ', $ 
                            event_pro='iris_xcontrol_leve3file')
  hdrdispmenu=widget_button(optmenu, value='Display header', $
			    event_pro='iris_xcontrol_hdrdisp')
...
;  lslabel = widget_label(lineselect_label, value=title)
  ...
  if ct gt 0 then begin
    ...
    lineselect_fuv = cw_bgroup(lineselect_row, /nonexclusive, $
			   line_id_fuv, event_func = 'iris_xcontrol_lineselect_fuv')
  ...
  if ct gt 0 then begin
    ...
    lineselect_nuv = cw_bgroup(lineselect_row, /nonexclusive, $
			   line_id_nuv, event_func = 'iris_xcontrol_lineselect_nuv')
  ...
  dispselect = cw_bgroup(dispselect_label, row = 5, $
			 modes,  event_func = 'iris_xcontrol_dispselect')
  ...
  nuv_icon                   = widget_draw(nuv_icon_base  ,              $
				...
				frame       = 1, event_pro='iris_xcontrol_detnuv',/button_events) 
  ...
  fuv_icon                   = widget_draw(fuv_icon_base  ,              $
				...
				XSize       = icon_size,                 $
				YSize       = icon_size*icon_aspect,     $
				frame       = 1, event_pro='iris_xcontrol_detfuv',/button_events)  
  ...
  sji_nuv1_icon                   = widget_draw(sji_nuv_base  ,             $
				...
				XSize       = sji_size,                      $
				YSize       = sji_size,                      $
				frame       = 1, event_pro='iris_xcontrol_shownuv1',/button_events) 
  sji_nuv2_icon                   = widget_draw(sji_nuv_base  ,             $
             			retain = 2, $ 
				XSize       = sji_size,                      $
				YSize       = sji_size,                      $
				frame       = 1, event_pro='iris_xcontrol_shownuv2',/button_events) 
  sji_fuv1_icon                   = widget_draw(sji_nuv_base  ,             $
             			retain = 2, $ 
				XSize       = sji_size,                      $
				YSize       = sji_size,                      $
				frame       = 1, event_pro='iris_xcontrol_showfuv1',/button_events) 
  sji_fuv2_icon                   = widget_draw(sji_nuv_base  ,             $
             			retain = 2, $ 
				XSize       = sji_size,                      $
				YSize       = sji_size,                      $
				frame       = 1, event_pro='iris_xcontrol_showfuv2',/button_events) 
  ...
  pointing_icon= widget_draw(pointing_base  ,             $
				...
				XSize       = pointing_size,                      $
				YSize       = pointing_size,                      $
				frame       = 1, event_pro='iris_xcontrol_pointing',/button_events)  
  ...
  momplot  = widget_droplist(momfield, value = momnames, title=momtitle, $
				     event_pro = 'iris_xcontrol_listmoments')
  ...
  closebutton = widget_button(closefield, value = 'Close   ', $
	...
	printfilesbutton = widget_button(lcol, value = 'Print filename to console', $
			      event_pro = 'iris_xcontrol_printfilename')
	
  ; IRIS icon
  iris_icon_size=150
  iris_icon_aspect=146./200.
  iris_icon_base=widget_base(rcol,/col)
  iris_icon                   = widget_draw(iris_icon_base  ,                  $
				retain = 2,                                    $ 
				XSize       = iris_icon_size,                  $
				YSize       = iris_icon_size*iris_icon_aspect, $
				frame       = 1)  
  ...
  ;
  iris_xcontrol_get_data_info, info
  ...
  if nbad ne 0 then deticon_resized[bad]=data->missing()
;  nbad=n_elements(where(deticon_resized eq data->missing()))
;  fbad=float(nbad)/float(n_elements(deticon_resized))
  deticon_min = min(iris_histo_opt(deticon_resized,0.005,missing=data->missing())>1.e-4)
  deticon_max = max(iris_histo_opt(deticon_resized,0.005,missing=data->missing()))
...
;  nbad=n_elements(where(deticon_resized eq data->missing()))
;  fbad=float(nbad)/float(n_elements(deticon_resized))
  deticon_min = min(iris_histo_opt(deticon_resized,0.01,/bot_only,missing=data->missing()))
  deticon_max = max(iris_histo_opt(deticon_resized,0.001,/top_only,missing=data->missing()))
  deticon_mean=mean(deticon_resized)-deticon_min
  loadct,3
...
; check if we have a valid date, no SDO icon if not
  ...
  help,date_obs
  date_obs='2018-04-21T12:15:22.847'
...
; compute corners of raster
  ...
  if not showim then begin
    ...
    xraster=xcen[0]
    yraster=ycen[0]
    theta=data->getinfo('SAT_ROT')*180./!pi
  ...
  xmanager, 'iris_xcontrol', tlb, /no_block, $
	    group_leader = group, cleanup = 'iris_xcontrol_cleanup'
end
