function iris_abscal,dnps0,wave0 ,date,fac=fac,version=version,verbose=verbose,cgs=cgs,debug=debug
;+
;   intensity=iris_abscal(dnps,wave ,date,fac=fac,version=version,/cgs,/verbose,/debug)
;
...
if(n_elements(version) ne 0) then begin
  if(fix(version) eq 3) and (n_elements(date) eq 0) then begin
...
if(count gt 0) then begin
  ...
  if(n_elements(version) gt 0) then begin
    if(fix(version) ge 3) then dn2phot[iw]=iresp.dn2phot_sg[0] else dn2phot[iw]=4.
  endif else begin
    dn2phot[iw]=iresp.dn2phot_sg[0]
  endelse
  pix_xl[iw]=0.01298
endif
...
if(count gt 0) then begin
  ...
  if(n_elements(version) gt 0) then begin
    if(fix(version) ge 3) then dn2phot[iw]=iresp.dn2phot_sg[1] else dn2phot[iw]=18.
  endif else begin
    dn2phot[iw]=iresp.dn2phot_sg[1]
  endelse
  pix_xl[iw]=0.02546
endif
